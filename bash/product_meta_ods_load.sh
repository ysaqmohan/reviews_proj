...
PROJDIR="/edw/Reviews"
export PROJDIR
script_name="product_meta_ods_load"
...
wget -O $PROJDIR/SourceCode/data/asin_metadata.json.gz https://s3-eu-west-1.amazonaws.com/bigdata-team/job-interview/metadata.json.gz
error_check

gzip $PROJDIR/SourceCode/data/asin_metadata.json.gz
error_check

if [ ! -f $PROJDIR/SourceCode/data/asin_metadata.json ]; then
...
#PySpark load
$PROJDIR/spark-2.4.5-bin-hadoop2.7/bin/spark-submit $PROJDIR/SourceCode/python/product_meta_ods_load.py >> $log_file 2>&1
error_check

echo "###################################################################################################################################"  >> $log_file
echo "###################################################################################################################################"  >> $log_file
echo "Create indices on target table" >> $log_file
echo "###################################################################################################################################"  >> $log_file
echo "###################################################################################################################################"  >> $log_file
